...
struct ContentCreditCard: View {
    /// View Properties
    @State private var card: CreditCard = .init()
    ...
    func CardFrontView() -> some View {
        VStack(alignment: .leading, spacing: 15) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                /// Let's display a temporary 16-digit * that will be replaced with the actual
                /// value as you type
    ...
    func group(_ character: Character, count: Int) -> String {
        ...
        for index in 0..<modifiedString.count {
            if index % count == 0 && index != 0 {
                ...
                let stringIndex = modifiedString.index(modifiedString.startIndex, offsetBy:
                    index + groupCharactersCount)
